...
/**
 * Authenticated, ownership- and unlock-gated delivery of a pack's on-disk
 * assets. This REPLACES the previous unconditional `useStaticAssets` mount:
 * the locked stickers (index >= freeStickerCount, before the per-pack referral
 * unlock) are the product's monetisation gate, so their bytes must never be
 * served to anyone but the owner of an unlocked pack.
 *
...
export class StickerFileController {
  ...
  async serve(
    ...
    // Gate locked stickers behind the per-pack referral unlock. Sticker files
    // are 1-based (sticker_<n>.webp), exposed to clients as 0-based index n-1;
    // the first freeStickerCount are always free, the rest need unlockedAt.
    if (match) {
      const index = Number(match[1]) - 1;
      const unlocked = pack.unlockedAt != null;
      if (!unlocked && index >= this.offer.freeStickerCount) {
        // Do not distinguish "locked" from "missing" — both 404.
