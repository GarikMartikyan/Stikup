...
import { SOUND_HAPTIC, type SoundEvent } from "./manifest";

/**
 * Fire the Telegram HapticFeedback call corresponding to the given sound event.
 * No-ops silently when HapticFeedback is unavailable (outside Telegram or old
 * client). Never throws.
 */
export function triggerHaptic(event: SoundEvent): void {
  try {
